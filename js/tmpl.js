...
table_templates = {
    ...
    mine: '<tr id="row_<%= id %>" class="<%= even %>" data-project-id="<%= company %>"><td><%= company %></a></td><td><%= registration_number %></td><td><a href="<%= invest_agree_url %>" target="_new"><%= invest_agree %></a></td><td><%= soumnamemo %></td><td><%= aimagnamemo %></td><td><%= centralpay %></td><td><%= localpay %></td><td><%= totalpay %></td></tr>'
};
