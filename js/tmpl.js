...
table_templates = {
    ...
    mine: '<tr id="row_<%= id %>" class="<%= even %>" data-project-id="<%= company %>"><td><%= company %></a></td><td><%= registration_number %></td><td><%= invest_agree %></td><td><%= soumnamemo %></td><td><%= aimagnamemo %></td><td><%= centralpay %></td><td><%= localpay %></td><td><%= totalpay %></td></tr>'
};
